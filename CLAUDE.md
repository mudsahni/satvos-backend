...
- **VoucherBuilderService**: Converts a parsed Document into a `VoucherDef` with `MatchConfidence` map. Three-tier party ledger matching: GSTIN exact match → normalized name match against existing Tally ledgers → convention name fallback. Determines `VoucherMode` (`accounting_invoice` | `item_invoice` | `journal`) based on matched inventory items and GST presence. Populates `SupplierInvoiceNo`, `SupplierInvoiceDate`, and `PartyDetails` (Name, Address, PAN, GSTIN, State) for rich ledger creation in Tally. Tax rates snapped to standard GST rates (0.25, 1.5, 2.5, 3, 5, 6, 7.5, 9, 12, 14, 18, 28) to avoid floating point noise. Inventory items only included when a matching stock item exists in Tally masters (service invoices become accounting-only vouchers). Date normalization from DD-MM-YYYY/DD/MM/YYYY to YYYY-MM-DD. VoucherMode constants in `domain/enums.go`
